...
async def create(request: UserLoginSchema, db: Session = Depends(get_db())):
    ...
    return Response(code=200, status="Ok", message="Book created successfully").dict(
        exclude_none=True
    )
...
async def get(db: Session = Depends(get_db())):
    ...
    return Response(
        code=200, status="OK", message="Success fetch all data", result=_user
    ).dict()
...
async def get_by_id(id: int, db: Session = Depends(get_db)):
    ...
    return Response(
        code=200, status="Ok", message="Success get data", result=_user
...
async def delete(id: int, db: Session = Depends(get_db())):
    ...
    return Response(code=200, status="OK", message="Success delete data").dict(
        exclude_none=True
    )
